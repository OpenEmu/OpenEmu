...
void LCD::power() {
  ...
  foreach(n, screen) n = 0x0000;
  foreach(n, line) n = 0x0000;
  foreach(n, origin) n = Origin::None;

  foreach(n, vram) n = 0x00;
  foreach(n, oam) n = 0x00;
  foreach(n, bgp) n = 0x00;
  foreach(n, obp[0]) n = 0x00;
  foreach(n, obp[1]) n = 0x00;
  foreach(n, bgpd) n = 0x0000;
  foreach(n, obpd) n = 0x0000;
