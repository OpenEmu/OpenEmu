...
static uint16_t conv_buf[224 * 144] __attribute__((aligned(16)));
static uint32_t mednafen_buf[224 * 144] __attribute__((aligned(16)));
static bool failed_init;

void retro_init()
{
    MDFN_PixelFormat pix_fmt(MDFN_COLORSPACE_RGB, 16, 8, 0, 24);
    surf = new MDFN_Surface(mednafen_buf, 224, 144, 224, pix_fmt);
...
bool retro_load_game(const struct retro_game_info *info)
{
    if (failed_init)
        return false;
    game = MDFNI_LoadGame("wswan", info->path);
    return game;
}
...
static inline void convert_surface()
{
    ...
    for (unsigned i = 0; i < 224 * 144; i += 8)
    {
