...
Byte k7zSignature2[k7zSignatureSize] = {'7', 'z', 0xBC, 0xAF, 0x27, 0x1C};
...
void SzCoderInfo_Init2(CSzCoderInfo *p)
{
  Buf_Init(&p->Props);
}

void SzCoderInfo_Free2(CSzCoderInfo *p, ISzAlloc *alloc)
{
  Buf_Free(&p->Props, alloc);
  SzCoderInfo_Init2(p);
}

void SzFolder_Init2(CSzFolder *p)
{
  p->Coders = 0;
  p->BindPairs = 0;
  p->PackStreams = 0;
...
void SzFolder_Free(CSzFolder *p, ISzAlloc *alloc)
{
  ...
  if (p->Coders)
    for (i = 0; i < p->NumCoders; i++)
      SzCoderInfo_Free2(&p->Coders[i], alloc);
...
static int TestSignatureCandidate(Byte *testBytes)
{
  ...
  for (i = 0; i < k7zSignatureSize; i++)
    if (testBytes[i] != k7zSignature2[i])
      return 0;
  return 1;
}
...
static SRes SzGetNextFolderItem(CSzData *sd, CSzFolder *folder, ISzAlloc *alloc)
{
  ...
  for (i = 0; i < numCoders; i++)
    SzCoderInfo_Init2(folder->Coders + i);
