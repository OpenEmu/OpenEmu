...
void program_write_byte_8(UINT32 addr, UINT8 byte);
void io_write_byte_8(UINT32 addr, UINT8 byte);

UINT8 program_read_byte_8(UINT32 addr);
UINT8 io_read_byte_8(UINT32 addr);
...
INLINE UINT32 ARG16(void)
{
	unsigned pc = PCD;
	PC += 2;
	return program_read_byte_8(pc) | (program_read_byte_8((pc+1)&0xffff) << 8);
}
...
#define JP {													\
	...
	{															\
		UINT8 op = program_read_byte_8(PCD);								\
		if( PCD == oldpc-1 )									\
		{														\
			/* NOP - JP $-1 or EI - JP $-1 */					\
			if ( op == 0x00 || op == 0xfb )						\
...
void z80_init()
{
	...
	if( !SZHVC_add || !SZHVC_sub )
	{
		...
		if( !SZHVC_add || !SZHVC_sub )
		{
			printf("Z80: failed to allocate 2 * 128K flags arrays!!!");
            assert(0);
		}
