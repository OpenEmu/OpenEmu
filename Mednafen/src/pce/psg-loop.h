  if(ch->control & 0x40) // DDA mode
  {
    #ifdef LFO_ON
    just_do_dda_lfo:
    WaveSynth.offset(timestamp, ch->dda_cache[0] - ch->last_dda_cache[0], &sbuf[0]);
    WaveSynth.offset(timestamp, ch->dda_cache[1] - ch->last_dda_cache[1], &sbuf[1]);
  ...
  else if(ch->noisectrl & 0x80)
  {
   ...
   {
    ch->dda = (ch->lfsr & 0x1) ? 0x1F : 0;

    CLOCK_LFSR(ch->lfsr);

    redo_ddacache(ch);
    NoiseSynth.offset_inline(timestamp + ch->noisecount, ch->dda_cache[0] - ch->last_dda_cache[0], &sbuf[0]);
    NoiseSynth.offset_inline(timestamp + ch->noisecount, ch->dda_cache[1] - ch->last_dda_cache[1], &sbuf[1]);
    ch->last_dda_cache[0] = ch->dda_cache[0];
    ch->last_dda_cache[1] = ch->dda_cache[1];
    ch->noisecount += freq;
   }
...
//	printf("Weehee %d %d %d %d\n", run_time, psg.channel[1].counter, ch->counter, ch->frequency);
    ...
    redo_ddacache(ch);
    #ifdef LFO_ON
    WaveSynth.offset(timestamp + ch->counter - run_time, ch->dda_cache[0] - ch->last_dda_cache[0], &sbuf[0]);
    WaveSynth.offset(timestamp + ch->counter - run_time, ch->dda_cache[1] - ch->last_dda_cache[1], &sbuf[1]);
    #else
    WaveSynth.offset_inline(timestamp + ch->counter, ch->dda_cache[0] - ch->last_dda_cache[0], &sbuf[0]);
    WaveSynth.offset_inline(timestamp + ch->counter, ch->dda_cache[1] - ch->last_dda_cache[1], &sbuf[1]);
    #endif
    ch->last_dda_cache[0] = ch->dda_cache[0];
    ch->last_dda_cache[1] = ch->dda_cache[1];
    ch->counter += freq;
