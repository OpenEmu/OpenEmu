...
#import "OEGameWindow.h"
#import "NSImage+OEDrawingAdditions.h"

#import "OEImageButton.h"
@interface OEGameWindow (Private)
- (void)_setup;
@end
@implementation OEGameWindow

+ (void)initialize{
	NSImage* img = [NSImage imageNamed:@"gamewindow"];
	
	[[img setName:@"game_window_active" forSubimageInRect:NSMakeRect(0, 0, img.size.width/2, img.size.height)] setFlipped:TRUE];
	[[img setName:@"game_window_inactive" forSubimageInRect:NSMakeRect(img.size.width/2, 0, img.size.width/2, img.size.height)] setFlipped:TRUE];
}
...
- (void)setContentSize:(NSSize)newSize
{
	...
	OEGameWindowThemeView *frameView = [super contentView];
...
- (void)setContentView:(NSView *)aView
{
	...
	if (!frameView)
	{
		frameView = [[[OEGameWindowThemeView alloc] initWithFrame:bounds] autorelease];
		...
		[pushCell setImage:[NSImage imageNamed:@"game_window_close_button"]];
		[pushCell release];
...
@implementation OEGameWindowThemeView
- (NSRect)resizeRect{
...
- (void)drawRect:(NSRect)dirtyRect{
	...
	NSImage* borderImage = isFocued ? [NSImage imageNamed:@"game_window_active"] : [NSImage imageNamed:@"game_window_inactive"];
