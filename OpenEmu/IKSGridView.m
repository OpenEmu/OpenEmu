...
- (void)clipViewFrameChanged:(NSNotification*)notification{
	...
	if (![self inLiveResize]) { [self.gridLayer setNeedsLayout]; }
...
- (void)mouseDragged:(NSEvent *)theEvent
{
	...
	if(self.autoscrollTimer){ [self.autoscrollTimer invalidate]; self.autoscrollTimer=nil; }
	...
	if (!self.allowsMultipleSelection || NSEqualPoints(NSZeroPoint, mouseDownPoint)) { return; }
	...
	if (location.x < startPoint.x)
{
	...
	if (shiftKeyDown && [currentSelectedLayers count]) { 
		...
		CALayer *clickedLayer = [self _layerAtPoint:location];
		if (![clickedLayer isKindOfClass:[IKSGridItemLayer class]]) { return; }
